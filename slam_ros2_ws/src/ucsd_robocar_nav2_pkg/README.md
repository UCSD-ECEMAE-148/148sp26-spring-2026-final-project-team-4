...
## Indoor SLAM Workflow

Use this package in two stages for indoor autonomy:

1. **Explore and build a map**

`ros2 launch ucsd_robocar_nav2_pkg navigation.launch.py`

This starts robot state publishing, local EKF fusion, SLAM Toolbox, and the Nav2 planning stack so the robot can explore while building the map.

2. **Save the map**

`ros2 run nav2_map_server map_saver_cli -f ~/maps/my_indoor_map`

3. **Localize and navigate on the saved map**

`ros2 launch ucsd_robocar_nav2_pkg localization.launch.py map_yaml:=/absolute/path/to/my_indoor_map.yaml`

4. **Visualize the robot and the map in RViz**

`ros2 launch ucsd_robocar_nav2_pkg visualization.launch.py`

If you want to view live mapping while the robot explores, launch `navigation.launch.py` in one terminal and `visualization.launch.py` in another.

Sensor expectations:

- 2D LiDAR on `/scan` for SLAM, AMCL, and costmaps
- IMU on `/imu/data` for stable local orientation and angular rate estimates
- Depth camera visual odometry on `/stereo_inertial_odometry` for the local EKF, if your camera driver publishes it

If your depth camera publishes a different odometry or point cloud topic, update [config/ekf_local.yaml](config/ekf_local.yaml) and, if needed, the Nav2 costmap parameters.

The older GPS-related config files are legacy and are not used by the indoor launch files.

Gazebo is not wired in yet. RViz is the simplest working visualization path for the current indoor SLAM stack because it can show the map, scan, camera image, TF tree, and goal poses without adding a full robot simulation model and plugin set.
