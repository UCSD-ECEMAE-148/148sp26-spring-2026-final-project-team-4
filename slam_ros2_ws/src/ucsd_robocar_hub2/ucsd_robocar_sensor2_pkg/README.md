...
Launch file that will start the IMU node provided by the manufacturer, used here for the Seeed Technology Co., Ltd. Seeed XIAO nRF52840 Sense, with the configuration specified in the [my_razor](#my_razor) config file.

On ROS Jazzy, the driver also requires the `ros-jazzy-tf-transformations` package and a source checkout of `razor_imu_9dof`.
